...
const CertifiedCourses = ({ course }) => {
  // Use related programs from course API if available
  const [courses, setCourses] = useState(course?.relatedPrograms || []);
  const [loading, setLoading] = useState(!(course?.relatedPrograms && course.relatedPrograms.length > 0));
  const [error, setError] = useState(null);

  useEffect(() => {
    // Only fetch if no related programs from course
    if (!course?.relatedPrograms || course.relatedPrograms.length === 0) {
      fetchCourses();
    } else {
      setCourses(course.relatedPrograms);
      setLoading(false);
    }
  }, [course?.relatedPrograms]);

  const fetchCourses = async () => {
    try {
      setLoading(true);
      const data = await api.get('/cms/certified-courses');
  ...
  return (
    ...
    <h2 className="w-full max-w-[486px] mt-10 sm:mt-16 md:mt-20 text-3xl sm:text-4xl md:text-5xl lg:text-[56px] font-['Plus_Jakarta_Sans'] font-bold mb-5 px-4 sm:px-6 md:px-8 lg:ml-10 lg:text-nowrap lg:-mt-30">
      Related <span className="text-[#FDB11F]">Program</span>
    </h2>
